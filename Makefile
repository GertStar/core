...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=gert \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=gertd \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/gertd ./cmd/gertd
endif

build-linux:
	mkdir -p $(BUILDDIR)
	docker build --no-cache --tag gertmoney/core ./
	docker create --name temp gertmoney/core:latest
	docker cp temp:/usr/local/bin/gertd $(BUILDDIR)/
	docker rm temp

build-linux-with-shared-library:
	mkdir -p $(BUILDDIR)
	docker build --tag gertmoney/core-shared ./ -f ./shared.Dockerfile
	docker create --name temp gertmoney/core-shared:latest
	docker cp temp:/usr/local/bin/gertd $(BUILDDIR)/
	docker cp temp:/lib/libwasmvm.so $(BUILDDIR)/
	docker rm temp

install: go.sum 
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/gertd
...
draw-deps:
	...
	@goviz -i ./cmd/gertd -d 2 | dot -Tpng -o dependency-graph.png
